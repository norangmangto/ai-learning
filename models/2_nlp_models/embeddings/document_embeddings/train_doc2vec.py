...
def compare_document_embeddings(documents, labels):
    ...
    dm_model = train_doc2vec_dm(tagged_docs, vector_size=100, window=5,
                                epochs=30)
    dm_time = time.time() - start_time

    start_time = time.time()
    dbow_model = train_doc2vec_dbow(tagged_docs, vector_size=100, window=5,
                                    epochs=30)
    ...
    w2v_vectors = np.array(
        [get_averaged_word2vec(doc, word2vec_model) for doc in documents]
    )
...
def visualize_document_embeddings(model, labels, n_docs=200, method="tsne"):
    ...
    doc_vectors = np.array(
        [model.dv[str(i)] for i in range(min(n_docs, len(model.dv)))]
    )
...
def infer_new_document(model, new_doc):
    ...
    for i in range(len(model.dv)):
        ...
        sim = np.dot(vector, doc_vec) / (
            np.linalg.norm(vector) * np.linalg.norm(doc_vec)
        )
...
def main():
    ...
    documents, labels = generate_document_corpus(
        n_documents=600, doc_length_range=(20, 50)
    )
    ...
    print(
        "  • Doc2Vec: Learns document-specific context, better semantic understanding"
    )
