...
def generate_anomaly_data(n_samples=1000, contamination=0.1, dataset_type="blobs"):
    ...
    if dataset_type == "blobs":
        ...
        X_normal, _ = make_blobs(
            n_samples=n_normal, centers=3, cluster_std=0.5, random_state=42
        )
...
def evaluate_performance(y_true, y_pred, scores):
    ...
    bars = axes[1].bar(
        metrics, values, color=["steelblue", "green", "orange"], edgecolor="black"
    )
...
def tune_contamination(X, y_true, contamination_values=[0.05, 0.1, 0.15, 0.2]):
    ...
    plt.tight_layout()
    plt.savefig(
        "isolation_forest_contamination_tuning.png", dpi=300, bbox_inches="tight"
    )
    plt.show()
...
def analyze_anomaly_scores(scores, y_true):
    ...
    print(
        f"Anomaly - Mean: {anomaly_scores.mean():.4f}, Std: {anomaly_scores.std():.4f}"
    )
...
def main():
    ...
    X, y_true = generate_anomaly_data(
        n_samples=1000, contamination=0.1, dataset_type="blobs"
    )
    ...
    best_contamination = tune_contamination(
        X_scaled, y_true, contamination_values=[0.05, 0.1, 0.15, 0.2]
    )
    ...
    model, y_pred, scores = train_isolation_forest(
        X_scaled, contamination=best_contamination, n_estimators=100
    )
