...
def compare_kernels(X, y_true, nu=0.1):
    ...
    for idx, kernel in enumerate(kernels):
        ...
        axes[idx].contourf(xx, yy, Z, levels=np.linspace(Z.min(), 0, 7),
        ...
        axes[idx].scatter(X[:, 0], X[:, 1], c=colors, alpha=0.6,
...
def tune_nu_parameter(X, y_true, nu_values=[0.01, 0.05, 0.1, 0.15, 0.2]):
    ...
    # F1 Score
    axes[0].plot(nu_vals, [r['f1_score'] for r in results],
    ...
    # ROC AUC
    axes[1].plot(nu_vals, [r['roc_auc'] for r in results],
    ...
    axes[2].plot(nu_vals, [r['n_support_vectors'] for r in results],
...
def compare_with_isolation_forest(X, y_true):
    ...
    if X.shape[1] == 2:
        ...
        axes[0].scatter(X[:, 0], X[:, 1], c=colors_svm, alpha=0.6,
                       edgecolors='black', linewidth=0.5, s=40)
        axes[0].set_title(f'One-Class SVM\nF1: {f1_svm:.3f}, ROC AUC: {roc_auc_svm:.3f}',
                         fontsize=12)
        ...
        axes[1].scatter(X[:, 0], X[:, 1], c=colors_if, alpha=0.6,
                       edgecolors='black', linewidth=0.5, s=40)
        axes[1].set_title(f'Isolation Forest\nF1: {f1_if:.3f}, ROC AUC: {roc_auc_if:.3f}',
                         fontsize=12)
    ...
    plt.plot(fpr_svm, tpr_svm, linewidth=2, label=f'One-Class SVM (AUC = {roc_auc_svm:.3f})')
    plt.plot(fpr_if, tpr_if, linewidth=2, label=f'Isolation Forest (AUC = {roc_auc_if:.3f})')
...
def main():
    ...
    best_nu = tune_nu_parameter(X_scaled, y_true,
    ...
    model, y_pred, scores = train_one_class_svm(X_scaled, nu=best_nu,
