...
def generate_anomaly_data(n_samples=2000, contamination=0.1, n_features=20):
    ...
    X_normal, _ = make_classification(
        ...
        n_informative=n_features-4, n_redundant=4,
...
def train_autoencoder(X_train, input_dim, encoding_dim=8, epochs=50, batch_size=32, lr=0.001):
    """
...
def analyze_reconstruction_errors(errors, y_true):
    ...
    axes[0].hist(anomaly_errors, bins=50, alpha=0.7, label='Anomaly',
    ...
    bp = axes[1].boxplot(data_to_plot, labels=['Normal', 'Anomaly'],
                         patch_artist=True)
...
def main():
    ...
    print(f"Test set: {len(X_test)} samples ({sum(y_test==1)} normal, {sum(y_test==0)} anomalies)")
