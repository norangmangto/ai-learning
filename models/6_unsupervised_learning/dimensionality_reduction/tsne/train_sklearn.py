...
def load_sample_data(dataset="digits"):
    ...
    if dataset == "digits":
        ...
        X = np.array(
            mnist.data.iloc[indices]
            if hasattr(mnist.data, "iloc")
            else mnist.data[indices]
        )
        y = np.array(
            mnist.target.iloc[indices]
            if hasattr(mnist.target, "iloc")
            else mnist.target[indices]
        )
        y = y.astype(int)
        print(
            f"Loaded MNIST sample: {
        X.shape[0]} samples, {
            X.shape[1]} features"
        )
...
def compare_with_pca(X, y, perplexity=30):
    ...
    start_time = time.time()
    tsne = TSNE(
        n_components=2, perplexity=perplexity, init="pca", random_state=42, verbose=0
    )
    X_tsne = tsne.fit_transform(X)
