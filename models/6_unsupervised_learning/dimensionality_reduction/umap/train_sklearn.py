...
def compare_with_pca_tsne(X, y):
    ...
    axes[1].set_title(
        f"t-SNE\nSilhouette: {tsne_sil:.3f}\n" f"Time: {tsne_time:.2f}s", fontsize=11
    )
    ...
    axes[2].set_title(
        f"UMAP\nSilhouette: {umap_sil:.3f}\n" f"Time: {umap_time:.2f}s", fontsize=11
    )
...
def supervised_umap_comparison(X, y):
    ...
    start_time = time.time()
    unsupervised = umap.UMAP(
        n_neighbors=15, min_dist=0.1, random_state=42, verbose=False
    )
    ...
    start_time = time.time()
    supervised = umap.UMAP(n_neighbors=15, min_dist=0.1, random_state=42, verbose=False)
