...
def plot_dendrogram(X, linkage_method='ward', max_display=30):
    ...
    plt.ylabel('Distance')
    plt.axhline(y=np.median(Z[-10:, 2]), color='r', linestyle='--',
...
def compare_linkage_methods(X, n_clusters=4):
    ...
    print("="*70)
    print(f"{'Method':<15} {'Silhouette':<15} {'Davies-Bouldin':<15} {'Calinski-H':<15}")
    print("-"*70)
...
def train_hierarchical_clustering(X, n_clusters=4, linkage='ward',
...
def main():
    ...
    visualize_clusters(X_scaled, labels,
                      title=f'Hierarchical Clustering (Ward, k={optimal_k})')
