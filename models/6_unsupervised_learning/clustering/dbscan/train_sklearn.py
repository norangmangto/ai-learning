...
def generate_sample_data(dataset_type='moons', n_samples=1000):
    ...
    if dataset_type == 'moons':
        ...
        X1, _ = make_blobs(n_samples=n_samples//2, centers=3, cluster_std=0.4, random_state=42)
...
def find_optimal_eps(X, min_samples=5, k=4):
    ...
    for p in percentiles:
        ...
        plt.axhline(y=eps_candidate, color='r', linestyle='--', alpha=0.5,
...
def compare_with_kmeans(X, n_clusters=None):
    ...
    for label, color in zip(unique_labels, colors):
        if label == -1:
            ...
            mask = (dbscan_labels == label)
            ax1.scatter(X[mask, 0], X[mask, 1], c=[color], s=50, alpha=0.8, edgecolors='k')
    ax1.set_title(f'DBSCAN ({n_dbscan_clusters} clusters, {list(dbscan_labels).count(-1)} noise points)')
    ...
    print(f"\n✓ DBSCAN: {n_dbscan_clusters} clusters, {list(dbscan_labels).count(-1)} noise points")
...
def main():
    ...
    visualize_clusters(X_scaled, labels,
                      title=f"DBSCAN (eps={eps:.3f}, min_samples=5)")
