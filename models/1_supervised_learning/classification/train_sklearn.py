...
def train():
    ...
    # Check 2: Model accuracy for iris dataset (SVM typically performs very well)
    if acc > 0.95:
        print(f"✓ Excellent accuracy: {acc:.4f} (> 0.95)")
    elif acc > 0.85:
        print(f"✓ Good accuracy: {acc:.4f} (> 0.85)")
    else:
        print(f"⚠ Moderate accuracy: {acc:.4f} (SVM usually performs very well on Iris)")
    ...
    print(f"Total Support Vectors: {sum(n_support)} out of {len(X_train)} training samples")
