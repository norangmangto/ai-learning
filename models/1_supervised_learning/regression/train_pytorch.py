...
def train():
    ...
    if r2 > 0.5:
        ...
        print(
            f"✗ WARNING: Poor R² score: {
        r2:.4f} (model performs worse than mean baseline)"
        )
    ...
    print("\n=== Overall Validation Result ===")
    validation_passed = (
        np.all(np.isfinite(y_pred_np))
        and r2 > 0
        and
        # MSE should be reasonable compared to variance
        mse < np.var(y_test_np) * 2
    )
