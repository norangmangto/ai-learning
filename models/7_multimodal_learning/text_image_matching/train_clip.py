...
def cross_modal_retrieval(model, images, texts, labels, k=5):
    ...
    for i in range(min(3, len(images))):
        ...
        for rank, idx in enumerate(top_k_indices, 1):
            print(f"  {rank}. Text {idx} (Class {labels[idx]}) - Similarity: {similarity[i, idx]:.3f}")
    ...
    for i in range(min(3, len(texts))):
        ...
        for rank, idx in enumerate(top_k_indices, 1):
            print(f"  {rank}. Image {idx} (Class {labels[idx]}) - Similarity: {similarity[idx, i]:.3f}")
...
def main():
    ...
    train_images, train_texts, train_labels = images[:n_train], texts[:n_train], labels[:n_train]
    val_images, val_texts, val_labels = images[n_train:n_train+n_val], texts[n_train:n_train+n_val], labels[n_train:n_train+n_val]
    test_images, test_texts, test_labels = images[n_train+n_val:], texts[n_train+n_val:], labels[n_train+n_val:]
