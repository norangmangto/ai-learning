...
def train():
    ...
    for epoch in range(epochs):
        ...
        plt.imsave(
            f"gan_images_tensorflow/epoch_{epoch+1}.png",
            gen_img[0, :, :, 0] * 127.5 + 127.5,
            cmap="gray",
        )
    ...
    d_fake_scores = (
        tf.nn.sigmoid(discriminator(generated_samples, training=False))
        .numpy()
        .flatten()
    )
    ...
    d_real_scores = (
        tf.nn.sigmoid(discriminator(real_samples, training=False)).numpy().flatten()
    )
    ...
    if 0.2 < d_fake_scores.mean() < 0.8:
        print(
            f"✓ Generator creating believable fakes: {
    ...
    validation_passed = (
        np.all(np.isfinite(generated_samples))
