...
def train():
    ...
    if 0.2 < d_fake_scores.mean() < 0.8:
        print(f"✓ Generator creating believable fakes: discriminator score = {d_fake_scores.mean():.4f}")
    elif d_fake_scores.mean() < 0.2:
        print(f"⚠ Generator struggling: discriminator easily detects fakes ({d_fake_scores.mean():.4f})")
    else:
        print(f"⚠ Possible mode collapse: discriminator score too high ({d_fake_scores.mean():.4f})")
    ...
    if gen_std > 0.1:
        ...
        print(f"⚠ WARNING: Low variance in generated images: {gen_std:.4f} (possible mode collapse)")
    
    # Overall validation result
    print("\n=== Overall Validation Result ===")
    validation_passed = (
        np.all(np.isfinite(generated_samples.numpy())) and
        np.all((d_fake_scores >= 0) & (d_fake_scores <= 1)) and
        d_real_scores.mean() > 0.5 and
        gen_std > 0.1 and
        0.1 < d_fake_scores.mean() < 0.9
    )
    
    if validation_passed:
        print("✓ Model validation PASSED - GAN is performing as expected")
    else:
        print("✗ Model validation FAILED - Review training dynamics and generated samples")
