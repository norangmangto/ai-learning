...
def train_model(
    model, train_loader, val_loader, epochs=50, lr=0.001, model_name="model"
):
    """Train model."""
...
def compare_directions(X_train, y_train, X_val, y_val, input_size, num_classes):
    ...
    models = {
        "Unidirectional LSTM": UnidirectionalLSTM(
            input_size, 128, 2, num_classes, dropout=0.2
        ),
        "Bidirectional LSTM": BidirectionalLSTM(
            input_size, 64, 2, num_classes, dropout=0.2, pooling="last"
        ),
        "BiLSTM + Mean Pooling": BidirectionalLSTM(
            input_size, 64, 2, num_classes, dropout=0.2, pooling="mean"
        ),
        "BiLSTM + Attention": BidirectionalLSTM(
            input_size, 64, 2, num_classes, dropout=0.2, pooling="attention"
        ),
    }
...
def visualize_attention(model, sequences, labels, class_names, n_samples=3):
    ...
    with torch.no_grad():
        for i in range(n_samples):
            ...
            axes[i, 0].set_title(
                f"Sequence (True: {class_names[labels[i]]}, "
...
def main():
    ...
    model = BidirectionalLSTM(
        input_size, 128, 2, num_classes, dropout=0.3, pooling="attention"
    )
    train_model(
        model,
        train_loader,
