...
class TransformerClassifier(nn.Module):
    def __init__(
        self, vocab_size, d_model, nhead, num_layers, num_classes, max_len=100
    ):
...
def train():
    ...
    if test_loss < loss.item() * 2:
        ...
        print(
            f"⚠ Possible overfitting: Test loss ({
        test_loss:.4f}) >> Train loss ({
            loss.item():.4f})"
        )
    ...
    if len(unique_preds) == NUM_CLASSES:
        ...
        len(unique_preds)} out of {NUM_CLASSES} classes"
        )
    ...
    if accuracy > random_baseline * 1.5:
        print(
            f"✓ Model performs significantly better than random ({
        accuracy:.4f} vs {
            random_baseline:.4f})"
        )
    elif accuracy > random_baseline:
        print(
            f"⚠ Model slightly better than random ({
        accuracy:.4f} vs {
            random_baseline:.4f})"
        )
    else:
    ...
    print("\n=== Overall Validation Result ===")
    validation_passed = (
        np.all((y_pred >= 0) & (y_pred < NUM_CLASSES))
        and accuracy > random_baseline
        and loss.item() < 2.0
    )
