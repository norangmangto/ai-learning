...
def train():
    ...
    print(
        classification_report(y_test, predictions, target_names=["Class 0", "Class 1"])
    )
    ...
    if test_accuracy > random_baseline * 1.2:
        print(
            f"✓ Model performs better than random ({
        test_accuracy:.4f} vs {
            random_baseline:.4f})"
        )
    ...
    validation_passed = (
        np.all((predictions >= 0) & (predictions < 2))
        and test_accuracy > random_baseline
        and final_train_loss < 2.0
    )
