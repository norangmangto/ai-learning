...
def content_loss(target_features, content_features):
    ...
    for layer in CONTENT_LAYERS:
        loss += nn.functional.mse_loss(target_features[layer], content_features[layer])
    return loss
...
def neural_style_transfer(content_path, style_path, device="cpu"):
    ...
    def closure():
        ...
        target_features = feature_extractor(target_img, CONTENT_LAYERS + STYLE_LAYERS)
...
def main():
    ...
    result_img, history = neural_style_transfer(
        content_path, style_path, CONFIG["device"]
    )
