...
from tensorflow.keras.layers import Dense, GlobalAveragePooling2D, LSTM, TimeDistributed, Dropout
...
def train():
    ...
    y_test_frames = tf.keras.utils.to_categorical(np.random.randint(0, num_classes, 20), num_classes)

    loss_frame, acc_frame = model_frame.evaluate(X_test_frames, y_test_frames, verbose=0)
    print(f"Frame-based model - Test Accuracy: {acc_frame:.2%}")

    X_test_sequences = np.random.randn(10, 16, 224, 224, 3).astype(np.float32)
    y_test_sequences = tf.keras.utils.to_categorical(np.random.randint(0, num_classes, 10), num_classes)

    loss_lstm, acc_lstm = model_lstm.evaluate(X_test_sequences, y_test_sequences, verbose=0)
